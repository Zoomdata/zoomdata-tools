...
class MySQLConnection(ZoomdataObject):
    # Create a new connection to an instance of MySQL
    def __init__(self, name, request, jdbc_url, connectionUser, connectionPassword, connectionTypeId=""):
        ...
        self.payload["subStorageType"] = "MYSQL"
        ...
        if connectionTypeId == "":
            ...
            self.payload["connectionTypeId"] = "SQL_MYSQL"
            self.payload["type"] = "SQL"
        else:
...
class MySQLDatasource(ZoomdataObject):
    # Create a new datasource for a collection(table) in MySQL
    def __init__(self, name, request, connectionID, collection, schema=None, customSQLFlag="false", connectorType="SQL"):
        ...
        # Read the request payload template into this object
        self.initPayload(template_directory+"/datasource/mysql.json")
        # Populate the request payload
        self.payload["name"] = name
        self.payload["type"] = connectorType # Should be set to "SQL" for legacy connections; "EDC2" for EDC
        self.payload["subStorageType"] = "MYSQL" # Not necessary for core connectors, but doesn't hurt
